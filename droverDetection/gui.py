...
class Display:
	...
	def run(self):
		...
		self.video.set_playback_state(self.stop_reading)

		frame_raw = self.video.read()
		if frame_raw is None:
			return

		frame_annotated, fgmask, hsv = self.detection.detect(frame_raw.copy(), self.frame_idx)
		...
		elif self.feed_option == VideoFeedOptions.OTHER:
			cv.imshow(self.windowName, hsv)
	...
	def wait_key(self, delay=30):
		...
		if key == ord("q") or key == 27:
			...
			self.feed_option = VideoFeedOptions.OTHER
		return True
