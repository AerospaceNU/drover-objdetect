...
def filter_hsv(frame, avg=None):
	...
	color_filter = cv.bitwise_not(
		cv.inRange(frame_HSV, (max(avg[0] - low_H, 0), max(avg[1] - low_S, 0), 0), (min(avg[0] + high_H, 180),
																					min(avg[1] + high_S, 255),
																					255)))
	value_filter = cv.inRange(frame_HSV, (0,
										  0,
										  max(avg[2] - low_V, 0)),
							  (180,
							   255,
							   min(avg[2] + high_V, 255)))
